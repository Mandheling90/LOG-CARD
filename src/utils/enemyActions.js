...
const BEGGING_VARIANTS = {
  money: {
    ...
    sihye: {
      heal: 20,
      strength: 2,
      debuffLog: "돈을 줘서 주머니가 가벼워졌다...",
    },
    attack: {
      debuff: { name: "빈곤의 저주", duration: 2, drawLimit: 3 },
      log: '"돈도 없는 놈이 칼질이야!"',
    },
    ignore: {
      block: 15,
      strength: 1,
      log: '"쳇, 쩨쩨한 놈. 돈으로 장비나 사야겠군"',
    },
  },
  food: {
    name: "밥구걸",
    ...
    attack: {
      debuff: { name: "굶주린 자의 원한", duration: 2, energyReduction: 1 },
      log: '"굶주린 자를 때리다니!"',
    },
...
export function processEnemyAction(enemy, enemyIndex, intent, state, bossRefs) {
  ...
  if (intent.type === "attack" || intent.type === "rage") {
    ...
    if (intent.stripBlock && (player.block || 0) > 0) {
      const ratio =
        typeof intent.stripBlock === "number" ? intent.stripBlock : 1;
      const stripped =
        ratio >= 1 ? player.block : Math.floor(player.block * ratio);
      player = { ...player, block: player.block - stripped };
      logs.push(
        `호신강기 ${ratio >= 1 ? "관통" : "파쇄"}! → 방어 ${stripped} 제거!`,
      );
    }
    ...
    for (let h = 0; h < hits; h++) {
      ...
      if (result.dodged && result.onEvadeDmg > 0) {
        ...
        enemies[enemyIndex] = {
          ...
          block: Math.max(
            0,
            (enemies[enemyIndex].block || 0) - result.onEvadeDmg,
          ),
      ...
      if (!result.dodged && result.counterDmg > 0) {
        ...
        enemies[enemyIndex] = {
          ...
          block: Math.max(
            0,
            (enemies[enemyIndex].block || 0) - result.counterDmg,
          ),
    ...
    if (intent.debuff && hpDamaged) {
      ...
      if (d.damageReceiveMultiplier)
        effects.push(
          `받는 피해 ${Math.round((d.damageReceiveMultiplier - 1) * 100)}% 증가`,
        );
    ...
    logs.push(
      `${enemy.name} 기공 운용! → 공력 +${intent.value}${intent.block ? `, 방어 +${intent.block}` : ""}`,
    );
    ...
    logs.push(
      `${enemy.name} 회복! → 체력 +${maxHeal}${healBlock > 0 ? `, 방어 +${healBlock}` : ""}`,
    );
    ...
    const effects = [];
    if (intent.value)
      effects.push(`받는 피해 ${Math.round((intent.value - 1) * 100)}% 증가`);
    ...
    logs.push(
      `${debuffName}! → ${effects.join(", ")} (${intent.duration || 2}턴)`,
    );
    ...
    if (sihyeUsed) {
      ...
      if (atkEffect.debuff) {
        ...
        if (d.drawLimit)
          logs.push(`→ 드로우 ${d.drawLimit}장 제한 (${d.duration}턴)`);
        else if (d.energyReduction)
          logs.push(`→ 기력 -${d.energyReduction} (${d.duration}턴)`);
        else if (d.damageReceiveMultiplier)
          logs.push(
            `→ 받는 피해 ${Math.round((d.damageReceiveMultiplier - 1) * 100)}% 증가 (${d.duration}턴)`,
          );
      }
      ...
      if (ignEffect.damageReduction) {
        ...
        logs.push(
          `${enemies[enemyIndex].name} → 받는 피해 ${Math.round(ignEffect.damageReduction * 100)}% 감소 (${ignEffect.damageReductionTurns || 2}턴)`,
        );
      }
      ...
      if (atkEffect.debuff) {
        ...
        if (debuff.drawLimit) {
          logs.push(
            `${debuff.name}! → 턴 드로우 ${debuff.drawLimit}장 제한 (${debuff.duration}턴)`,
          );
        } else if (debuff.energyReduction) {
          logs.push(
            `${debuff.name}! → 기력 -${debuff.energyReduction} (${debuff.duration}턴)`,
          );
        } else if (debuff.damageReceiveMultiplier) {
          logs.push(
            `${debuff.name}! → 받는 피해 ${Math.round((debuff.damageReceiveMultiplier - 1) * 100)}% 증가 (${debuff.duration}턴)`,
          );
        }
      }
      if (atkEffect.counterDmg) {
      ...
      if (ignEffect.damageReduction) {
        ...
        logs.push(
          `${enemy.name} → 받는 피해 ${Math.round(ignEffect.damageReduction * 100)}% 감소 (${ignEffect.damageReductionTurns || 2}턴)`,
        );
      }
    }
    sihyeUsed = false;
    bossAttacked = false;
  } else if (intent.type === "exhaustion") {
    // 탈력 상태: 받는 피해 2배 해제, 플레이어 공격 여부 확인
    enemies[enemyIndex] = {
      ...enemies[enemyIndex],
      damageReduction: 0,
    ...
    if (bossAttacked) {
      logs.push(`${enemy.name}의 빈틈을 찔렀다!`);
    } else {
      // 공격 안 함 → 미스 카운트 증가
      const missCount = (enemies[enemyIndex].exhaustionMissCount || 0) + 1;
      enemies[enemyIndex] = {
        ...enemies[enemyIndex],
        exhaustionMissCount: missCount,
      };
      const missLines = [
        `${enemy.name}: "...그르르!" 거친 숨을 내쉬며 기력을 되찾는다.`,
        `${enemy.name}: "...동정인가?" 눈빛이 날카로워진다.`,
        `${enemy.name}: "...그래, 베지 않는 것도 검이지." 마기가 잠잠해진다.`,
      ];
      logs.push(missLines[missCount - 1] || missLines[missLines.length - 1]);
      // 3회 미공격 → 각성
      if (missCount >= 3 && enemies[enemyIndex].phase2) {
        const p2 = enemies[enemyIndex].phase2;
        enemies[enemyIndex] = {
          ...enemies[enemyIndex],
          name: p2.name,
          emoji: p2.emoji,
          hp: Math.max(enemies[enemyIndex].hp, p2.hp),
          actions: p2.actions,
          bossPhase: 2,
          exhaustionMissCount: 0,
        };
        logs.push(
          `"지금 적을 앞두고 손속에 자비를 배푸는 것인가?"`,
          `"...시험해보고 싶은게 있다. 따라와라"`,
          `${p2.name}(으)로 각성! 검에서 파사의 기운이 뿜어져 나온다!`,
        );
      }
    }
    bossAttacked = false;
