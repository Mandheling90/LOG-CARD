...
function getCardStance(card) {
  for (const e of card.effects) {
    if (e.type === 'damage' || e.type === 'finisher' || e.type === 'aoe') return 'attack'
...
export function processCardEffects(card, state, targetIndex) {
  let { player, enemies, taeguk, buffs, evasionCount, evasionChance, counter, stance, logs } = { ...state }
  player = { ...player }
  enemies = enemies.map(e => ({ ...e }))
  logs = [...logs]

  const taegukMult = buffs.reduce((m, b) => b.taegukMultiplier ? m * b.taegukMultiplier : m, 1)
...
export function processEnemyAttack(damage, state) {
  ...
  // 받는 피해 증가 버프 체크
  const dmgMult = buffs.reduce((m, b) => b.damageReceiveMultiplier ? m * b.damageReceiveMultiplier : m, 1)
  ...
  return { player, evasionCount, evasionChance, counter, logs, dodged: false, counterDmg }
}
